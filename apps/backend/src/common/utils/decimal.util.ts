...
export class DecimalUtil {
  ...
  static subtract(minuend: number | string, subtrahend: number | string): number {
    return new Decimal(minuend || 0).minus(new Decimal(subtrahend || 0)).toNumber();
  }
  ...
  static multiply(
    ...
    return Number(
      new Decimal(multiplicand || 0).times(new Decimal(multiplier || 0)).toFixed(precision),
    );
  }
  ...
  static divide(
    ...
    return Number(new Decimal(dividend || 0).div(new Decimal(divisor)).toFixed(precision));
  }
   ...
   * @returns 用量
   */
  static calculateUsage(endRead: number | string, startRead: number | string): number {
    return this.subtract(endRead, startRead);
  }
   ...
   * @returns 费用（保留2位小数）
   */
  static calculateAmount(usage: number | string, rate: number | string): number {
    return this.multiply(usage, rate, 2);
  }

  /**
   * 计算余额（上期余额 + 应收金额 - 实收金额）
   * @param previousBalance 上期余额
   * @param receivable 应收金额
   * @param actualPaid 实收金额
   * @returns 新余额
   */
  static calculateBalance(
    ...
    return new Decimal(previousBalance || 0)
      .plus(new Decimal(receivable || 0))
      .minus(new Decimal(actualPaid || 0))
      .toNumber();
  }
   ...
   * @returns 总和
   */
  static sumArray<T>(values: T[], getter?: (item: T) => number | string): number {
    return values
      .reduce((sum, item) => {
        const value = getter ? getter(item) : (item as number | string);
        return sum.plus(new Decimal(value || 0));
      }, new Decimal(0))
      .toNumber();
  }
   ...
   * @returns 格式化后的数值
   */
  static format(value: number | string, precision: number = 2): number {
  ...
  static calculatePercentage(
    ...
    return Number(new Decimal(value || 0).div(new Decimal(total)).times(100).toFixed(precision));
  }
   ...
   */
  static compare(a: number | string, b: number | string): -1 | 0 | 1 {
